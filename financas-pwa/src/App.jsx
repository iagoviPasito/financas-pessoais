...
async function parsearPDF(arrayBuffer) {
  ...
  // Extrai todos os itens de todas as páginas com posição X/Y
  let todosItens = [];
  let textoSimples = "";
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    const itens = content.items
      .map(it => ({ texto: it.str.trim(), x: Math.round(it.transform[4]), y: Math.round(it.transform[5]), pag: i }))
      .filter(it => it.texto.length > 0);
    todosItens = todosItens.concat(itens);
    // Texto simples para detecção do banco
    const linhaMap = {};
    ...
    textoSimples += linhas.join("\n") + "\n";
  }

  // Detectar banco/tipo
  const isItauCard = /Lançamentos: compras e saques/i.test(textoSimples);
  const isBradCard = /Fatura Mensal.*bradesco|ELO NANQUIM|Histórico de Lançamentos/i.test(textoSimples);
  const isBradCC   = /Extrato de: Agência.*Conta.*Movimentação|Bradesco Celular/i.test(textoSimples);
  const isItauCC   = /extrato conta.*lançamentos|REND PAGO APLIC AUT MAIS/i.test(textoSimples);

  if (isItauCard) return extrairItauCartaoV2(todosItens, textoSimples);
  if (isBradCard) return extrairBradescoCartao(textoSimples);
  if (isBradCC)   return extrairBradescoCC(textoSimples);
  if (isItauCC)   return extrairItauCC(textoSimples);
  return extrairItauCartaoV2(todosItens, textoSimples);
}

// ── Itaú Cartão V2 — processa duas colunas separadamente por posição X ──
function extrairItauCartaoV2(itens, textoSimples) {
  // A fatura Itaú tem duas colunas de lançamentos lado a lado
  // Detectar o ponto de divisão X: mediana dos X únicos
  const xs = itens.map(it => it.x);
  const xMax = Math.max(...xs);
  // Divisor: metade da largura da página (~metade do X máximo)
  const xDivisor = Math.round(xMax * 0.52);

  // Separar itens por coluna e por Y
  const colunas = [
    itens.filter(it => it.x <= xDivisor),
    itens.filter(it => it.x > xDivisor),
  ];
  ...
  for (const col of colunas) {
    const linhaMap = {};
    col.forEach(it => { if (!linhaMap[it.y]) linhaMap[it.y] = []; linhaMap[it.y].push(it); });
    const linhas = Object.values(linhaMap)
      .sort((a,b) => b[0].y - a[0].y)
      .map(its => its.sort((a,b)=>a.x-b.x).map(it=>it.texto).join(" "));
  ...
  return extrairItauCartao(textoFinal);
}
...
const dadosIniciais = {
  transacoes: [
    { id: gerarId(), tipo:"gasto",  descricao:"Aluguel", obs:"", valor:1800, categoria:"Moradia",     mes:mesAtual, ano:anoAtual },
    { id: gerarId(), tipo:"gasto",  descricao:"Mercado", obs:"", valor:650,  categoria:"Alimentação", mes:mesAtual, ano:anoAtual },
    { id: gerarId(), tipo:"gasto",  descricao:"Gasolina",obs:"", valor:300,  categoria:"Transporte",  mes:mesAtual, ano:anoAtual },
    { id: gerarId(), tipo:"renda",  descricao:"Salário", obs:"", valor:5500, categoria:"Salário CLT", mes:mesAtual, ano:anoAtual },
    { id: gerarId(), tipo:"renda",  descricao:"Freela",  obs:"", valor:1200, categoria:"Freela/PJ",   mes:mesAtual, ano:anoAtual },
  ],
  metas: [
...
export default function App() {
  ...
  const [form, setForm]             = useState({ tipo:"gasto", descricao:"", obs:"", valor:"", categoria:CATEGORIAS_GASTO[0], mes:mesAtual, ano:anoAtual });
  ...
  const addTx = useCallback(() => {
    ...
    setTransacoes(p=>[...p,{...form, id:gerarId(), valor:parseFloat(form.valor), obs:form.obs||""}]);
    setForm(f=>({...f, descricao:"", obs:"", valor:""}));
  }, [form]);
  ...
  const handleArquivo = useCallback((e) => {
    ...
    const processarLinhas = (linhas) => {
      ...
      const comCat = linhas.map(l => {
        ...
        return { id:gerarId(), descricao:l.memo, obs:"", valor:Math.abs(l.valor), mes:isNaN(l.mes)?mesAtual:l.mes, ano:isNaN(l.ano)?anoAtual:l.ano, tipo:auto.tipo, categoria:auto.categoria, selecionado:true };
      });
      setPreview(comCat);
    };
    ...
    if (ext === "pdf") {
      ...
      reader.onload = async (ev) => {
        try {
          // Copia o buffer — Safari iOS detacha o ArrayBuffer após o primeiro uso
          const bufferOriginal = ev.target.result;
          const buffer1 = bufferOriginal.slice(0);
          const buffer2 = bufferOriginal.slice(0);

          const pdfjsLib = await carregarPdfJS();

          // Extrai texto para detecção de parcelas futuras (texto simples)
          const pdf = await pdfjsLib.getDocument({ data: buffer1 }).promise;
          let textoTotal = "";
          for (let i = 1; i <= pdf.numPages; i++) {
            const page = await pdf.getPage(i);
          ...
          // Processa transações com buffer separado (usa parser com colunas)
          const linhas = await parsearPDF(buffer2);
          processarLinhas(linhas);

          // Extrai parcelas futuras se for fatura Itaú
          const fp = extrairParcelasFuturas(textoTotal);
          if (fp.length > 0) {
            const comCat = fp.map(l => { const auto = categorizarAuto(l.memo, l.valor); return { id:gerarId(), descricao:l.memo, obs:"", valor:l.valor, mes:l.mes, ano:l.ano, tipo:"gasto", categoria:auto.categoria }; });
  ...
  const exportarExcel = useCallback(() => {
    ...
    const wsTx = XLSX.utils.json_to_sheet(transacoes.map(t=>({ Tipo:t.tipo==="gasto"?"Gasto":"Renda", Descrição:t.descricao, Observação:t.obs||"", Categoria:t.categoria, Valor:t.valor, Mês:MESES[t.mes], Ano:t.ano })));
        ...
        <div style={{ maxWidth:960, margin:"0 auto" }}>
          ...
          <div style={{ background:"#1a1710", border:"1px solid #2a2418", borderRadius:12, overflow:"hidden", marginBottom:20 }}>
            <div style={{ maxHeight:"55vh", overflowY:"auto" }}>
              {preview.map(p=>(
                <div key={p.id} style={{ borderBottom:"1px solid #1a1810", padding:"12px 16px", opacity:p.selecionado?1:0.35, background:p.selecionado?"transparent":"#0a0900" }}>
                  {/* Linha 1: checkbox + descrição + valor */}
                  <div style={{ display:"flex", alignItems:"flex-start", gap:10, marginBottom:8 }}>
                    <input type="checkbox" checked={p.selecionado} onChange={()=>togglePrev(p.id)} style={{ cursor:"pointer", accentColor:"#c8a96e", marginTop:3, flexShrink:0 }}/>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:13, color:"#e8dcc8", wordBreak:"break-word", lineHeight:1.4 }}>{p.descricao}</div>
                      <div style={{ fontSize:11, color:"#a09070", marginTop:2 }}>{MESES[p.mes]}/{p.ano}</div>
                    </div>
                    <span style={{ fontSize:14, fontWeight:"bold", color:p.tipo==="renda"?"#4a9e6a":"#e05040", flexShrink:0 }}>{p.tipo==="renda"?"+":"-"}{fmt(p.valor)}</span>
                  </div>
                  {/* Linha 2: categoria + tipo */}
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginLeft:26, marginBottom:6 }}>
                    <select value={p.categoria} onChange={e=>updatePrev(p.id,"categoria",e.target.value)} style={{...smInput, fontSize:12}}>
                      {[...CATEGORIAS_GASTO,...CATEGORIAS_RENDA].filter((v,i,a)=>a.indexOf(v)===i).map(c=><option key={c}>{c}</option>)}
                    </select>
                    <select value={p.tipo} onChange={e=>updatePrev(p.id,"tipo",e.target.value)} style={{...smInput, fontSize:12, color:p.tipo==="renda"?"#4a9e6a":"#e05040"}}>
                      <option value="gasto">💸 Gasto</option>
                      <option value="renda">💰 Renda</option>
                    </select>
                  </div>
                  ...
                  <div style={{ marginLeft:26 }}>
                    <input value={p.obs||""} onChange={e=>updatePrev(p.id,"obs",e.target.value)} style={{...smInput, fontSize:12, width:"100%", boxSizing:"border-box"}} placeholder="Observação (opcional)..."/>
                  </div>
                </div>
              ))}
            </div>
        ...
        {aba==="lancamentos" && (
          <div style={{ display:"grid", gridTemplateColumns:"320px 1fr", gap:24 }}>
            <div style={card}>
              ...
              <Lbl>Observação <span style={{color:"#504030",fontWeight:"normal"}}>(opcional)</span></Lbl>
              <input value={form.obs||""} onChange={e=>setForm(f=>({...f,obs:e.target.value}))} style={inp} placeholder="Ex: Parcela 2/12, referente a março..." onKeyDown={e=>e.key==="Enter"&&addTx()}/>
              <Lbl>Valor (R$)</Lbl>
                ...
                {txMes.length===0?<Vazio/>:txMes.map(t=>(
                  <div key={t.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:"1px solid #1e1c14" }}>
                    <div style={{ flex:1, minWidth:0, marginRight:12 }}>
                      <div style={{ fontSize:13, color:"#e8dcc8", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{t.descricao}</div>
                      {t.obs && <div style={{ fontSize:11, color:"#a08060", fontStyle:"italic", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{t.obs}</div>}
